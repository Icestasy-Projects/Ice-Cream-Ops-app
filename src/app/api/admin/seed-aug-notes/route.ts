...
const PREP_BULK: [string, number][] = [
  ['Mysore Paak', 20], ['Tender Coconut', 15], ['Coffee', 20],
  ['Gulab Jamun', 20], ['Reshmi Paan', 20], ['Kaju Katti', 20],
  ['Salted Caramel', 20], ['Kesar Thandai', 20], ['Khajoor', 10],
];

const RM_RECEIVED: [string, number][] = [
  ['13% IPPL Base', 50], ['Sugar', 50], ['Kishmish', 1],
  ['Rose Water', 0.6], ['Paan', 8], ['Milk', 8],
  ['Khoa', 0.8], ['Tender Coconut', 8.5], ['Maida', 0.5],
];

const PAGE1_STOCK: [string, number][] = [
  ['Tukda Kaju', 5], ['Fennel Seeds', 2.1], ['Orange Paste', 0.2],
  ['Orange Peel', 2], ['Saffron', 0.015], ['Almond', 10],
  ['Black Sesame', 5.7], ['Honey', 4],
];

const STOCK_COUNT_20AUG: [string, number][] = [
  ['Sugar', 43.8], ['13% IPPL Base', 12], ['White Butter', 10],
  ['Khoa', 0.38], ['Elaichi Powder', 0.3], ['High Fat Milk', 4],
  ['Sitaphal', 85], ['Biscoff', 104], ['Coffee Powder', 9],
  ['Mango Pulp', 109], ['Ghee', 27.64], ['Besan', 5.965],
  ['Maida', 0.724], ['Jaggery', 36], ['Coconut Butter', 28],
  ['Shredded Coconut', 17], ['Tender Coconut', 5], ['CEC Vanilla', 7.5],
  ['Vanilla Powder', 0.6], ['Dark Chocolate Slab', 18.3],
  ['JB 800 Cocoa Powder', 6.1], ['Choco Chip Cookie', 8],
  ['Cream', 21], ['Gulkand', 0.35], ['Dried Rose Petal', 2.8],
  ['Cashew Butter', 35],
];

export async function POST(req: NextRequest) {
  const { searchParams } = new URL(req.url);
  const section = searchParams.get('section') ?? 'prep';
  ...
  async function fuzzyFind(namePart: string) {
    const { data } = await s().from('rm_items').select('id, name, unit')
      .ilike('name', `%${namePart}%`).limit(1);
  ...
  if (section === 'prep') {
    ...
    for (const [flavour, bulkTubs] of PREP_BULK) {
      ...
      const { error } = await s().from('prep_units').insert({
        ...
        status: 'posted',
        note: `${bulkTubs} bulk tubs (${(totalLitres / batchYieldL).toFixed(1)} batches × ${batchYieldL}L)`,
      });
    ...
    for (const [namePart, qty] of RM_RECEIVED) {
      ...
      const { data: order, error: oErr } = await s().from('rm_purchase_orders').insert({
        vendor_id: vendorId, ordered_at: new Date().toISOString(),
        status: 'received', note: 'Page 1 notes — order received',
      }).select('id').single();
      if (oErr || !order) { log.push(`❌ PO for ${item.name}: ${oErr?.message}`); continue; }
      const { error } = await s().from('rm_purchase_order_lines').insert({
        order_id: (order as { id: number }).id, rm_item_id: item.id,
        qty_ordered: qty, qty_received: qty, unit_cost: null,
      });
