...
export async function GET() {
  try {
    ...
    if (formatIds.length > 0) {
      ...
      const skuIds = skus.map(s => s.id as number);
      if (skuIds.length > 0) {
    ...
    return NextResponse.json({
      ...
      would_delete: { formats: formats || [], skus, fg_skus: fgStock },
    });
  } catch (e: unknown) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const formatIds = (formats || []).map((f: Record<string, unknown>) => f.id as number);
    if (formatIds.length === 0) return NextResponse.json({ message: 'Nothing to delete — formats not found.' });
    ...
    const skuIds = (skuData || []).map((s: Record<string, unknown>) => s.id as number);
    const log: string[] = [];

    if (skuIds.length > 0) {
      // Remove FG ledger entries
      const { error: ledgerErr } = await admin
        .schema('production').from('fg_ledger')
        .delete()
        .in('fg_sku_id', skuIds);
      if (ledgerErr) throw new Error(`fg_ledger delete: ${ledgerErr.message}`);
      log.push('Cleared fg_ledger rows for these SKUs');
      ...
      log.push(`Deleted ${skuIds.length} SKU(s)`);
    }
    ...
    log.push(`Deleted pack formats: ${TARGET_FORMATS.join(', ')}`);

    return NextResponse.json({
      success: true,
      deleted: { formats: formats || [], skus: skuData || [] },
      log,
    });
  } catch (e: unknown) {
    return NextResponse.json({ error: e instanceof Error ? e.message : String(e) }, { status: 500 });
